...
def img_to_array_old(image_file, resize=84, flip=False, convert_to_gray=False):
    ...
    if flip:  # when reading a stack saved in vaa3d format Y coordinates are reversed (used by save_subst
...
def convert_mini_imagenet(_folder=None, resize=None):
    for st in ['train', 'val', 'test']:
        ...
        h5file = h5py.File(os.path.join(_folder or em.load.MINI_IMAGENET_FOLDER_RES84, '%s.h5' % st), 'w')
